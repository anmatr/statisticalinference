...
- $VarA$ = 2.0mg, $VarB$ = 1.0mg
- $VarB$ = 1.0mg, $VarC$ = 0.5mg
...
testAB
```
The 95% test interval does not contain zero, so we can reject the hypothesis that $VarA$ = $VarB$. That means the 2.0mg dose and the 1.0mg have the same effect on tooth growth.
```{r}
testBC
```
The 95% test interval also does not contain zero, so we can also reject the hypothesis that $VarB$ = $VarC$. That means the 1.0mg dose and the 1.0mg had also the same effect on thooth growth.
Using transitivity we can also reject the hypothesis that the 2.0mg and the 0.5mg does have the same effect on tooth growth.
As we rejected all 3 hypothesis we can asume with 95% confidence that higher doses of Vitamin C increase tooth growth in guinea pigs.
